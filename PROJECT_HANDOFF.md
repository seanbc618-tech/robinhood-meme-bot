...
## 最新接手入口（2026-09-25 更新，优先于下方历史记录）

### 2026-09-25：Telegram 改为日报（`9fcd1c0`）

- 去掉每小时持仓消息（此前 3 天发了 64 条），改为每天北京时间 9 点后的第一轮发一份日报，覆盖上一份之后的全部时间：运行是否正常及期间失败轮数、三个账户相对 1000 的盈亏、平仓（币名、盈亏%、原因）与持仓、信号数和没买的原因（暂停中 / 这个币已经买过 / 手续费+滑点超 5% / 持有人数据没拿到 / 其他检查代码）。成交和熔断消息照旧即时发送；start、end、err、源故障类消息本来就被 `notifySafe` 过滤，从未发出。
- 状态字段 `run.last_daily_day`、`last_daily_at`、`last_daily_failed`；去重键 `abc:daily:<北京日期>`。
- 部署：09-25 23:31 CST，`current -> releases/9fcd1c0`，快照 `shared/deployment-9fcd1c0-before.json`，账户与实验字段逐项未变；第一份日报 23:31:50 已发出（覆盖过去 24 小时）。
- 09-25 的交易：A 两笔，CARDS -14.2%（止损）、NYMA +20.8%（回撤止盈）。A 995.42，三账户合计 -34.44。
...
- **部署**：09-24 16:38 CST，`current -> releases/d548276`，快照 `shared/deployment-d548276-before.json`，账户与实验字段逐项未变。换版前旧进程 RSS 3241 MB；新进程 5 分钟时 142 MB，10 分钟时 144 MB，在 120–145 MB 之间来回、未见上涨；跑满 31 小时时 318 MB（修前 20 小时 3.3 GB）。
- **停机那一轮记为失败（`failed_rounds` 77→78）**：systemd 停服务时把 SIGTERM 发给整个 cgroup，旧 worker 最后一轮里正在取 ETH 价格的 curl 被一起杀掉（stderr 为空），这轮记失败，与新代码无关。`err:` 类提醒被 `notifySafe` 过滤，并没有发到 Telegram（当时对用户说"可能收到提醒"是错的，09-25 已更正）。以后每次部署都可能这样。
...
- 本地：`<LOCAL_REPO>`，main HEAD 为本文档提交（其前 `9fcd1c0`）。**`f2ad4f8` 之后的提交待在 Mac 上 `git push origin main`**（本 session 环境无 GitHub 凭据）。
- SSH：`<VPS_USER>@<VPS_HOST>`，会话密钥（`authorized_keys` 注释 `cowork-session`）常驻。
- VPS：`current -> releases/9fcd1c0`，PID `1802949`（须重新核实），systemd 托管。`releases/392da39` **持有唯一真实 node_modules，勿删**（详见下文清理章节）。
- 发布前快照：`shared/deployment-{...,aae5c26,65aed59,df34290,d9fb80c,e0c1565,8c627cf,4205d54,3467e8b,d548276,9fcd1c0}-before.json`；延长实验前快照：`shared/experiment-extend-before.json`。

### 8 个回合的结果与最重要的发现

总计 **-33.02 / 3000（-1.1%）**：A 989.64（4 笔）、B 1009.57（5 笔）、C 967.77（8 笔）。按入场成本（手续费+滑点）排序：

| 成本 | 结果 | 持有 | 策略 |
|---|---|---|---|
| 2.77% | **+50.4%** 追踪止盈 | 185 分 | B |
| 4.49% | -13.9% 止损 | 7.9 分 | A |
| 4.58% | -18.6% 止损 | 44.8 分 | B |
| 5.27% | -21.0% 止损 | 4.9 分 | C |
| 5.81% | -20.4% 止损 | 3.9 分 | C |
| 6.17% | -30.3% 止损 | 3.6 分 | C |
| 6.23% | -20.6% 止损 | 2.7 分 | A |
| 6.59% | -35.6% 止损 | 6.7 分 | C |

**成本 ≥5% 的五笔全部亏损，平均 -25.6%，且全在 2.7–6.7 分钟内被止损**；成本 <5% 的三笔合计 +17.9%，持有 7.9/44.8/185 分钟。8 笔是小样本，不是证明，但方向一致且成本是开仓前就能免费拿到的池深度读数。

### 本轮四项改动

1. `aae5c26`（上一轮）退出滞后修复已被数据验证：止损实际偏离从 15.6 个百分点降到 1.0–3.6 个百分点。`exit_ticks` 正常计数。
2. `65aed59` **价格源**：ETH/USDT 改用 Coinbase 交易所 ticker（自带成交时间戳）。VPS 实测 ETH 源年龄中位 4.1 秒、最大 15 秒、0/4 超门槛；部署后生产读数 2.0–5.7 秒。此前 CoinGecko 中位 107 秒、43.6% 超 120 秒门槛，导致 3 天里 26 个通过安全检查的信号只有 5 个成交。
3. `65aed59` **稳定币判定**：USDG/USDT 不再按时钟卡，改为要求报价落在 0.98–1.02 且来源不超过 15 分钟；**ETH 仍是严格 120 秒**，`usd_source_age_sec` 也只反映 ETH。USDG 无秒级源（Coinbase 未上市，Kraken ticker 无时间戳且成交稀疏），故仍取自 CoinGecko。用户明确授权此项。
4. `df34290` **成本闸 8% → 5%**。8% 自部署起从未拦下任何信号（三天最贵的报价信号 6.59%），唯一效果是放进了昂贵的那一半。用户看过上表后明确授权调回。
...
### 2026-09-17：A 首次大赢；我把采集搞垮了又修回（`d9fb80c`）

**11 个回合：2 胜 9 负，合计 -27.86 / 3000。** A 新增两个回合：09-16 08:43（成本 2.18%，-20.9%，1.2 分钟止损）与 09-16 10:04（成本 2.17%，**+62.0%**，持有 325 分钟，追踪止盈）。A 账户 989.64 → **1001.99**。

- **剔掉已暂停的 C，A+B 合计 +11.56，是盈利的。** 暂停 C 的决定事后看是对的。
- 形态很清楚：**两笔盈利都持有数小时（185、325 分钟），九笔亏损全部在 1.2–7 分钟内被止损打掉。** 平均赢 +16.89、平均亏 -6.85，赔率约 2.5:1 —— 这种形态胜率到约 29% 即可打平，当前 18%（n=11，不构成结论）。
- 入场成本已不能单独区分输赢：+62.0% 的成本是 2.17%，而 -20.9% 的是 2.18%。成本闸仍值得保留（它筛掉的 5% 以上那批平均 -25.6%），但别再指望它预测单笔结果。

**我在 `3acf591` 里犯的错，已在 `d9fb80c` 回退：** 把持有人预热从 8 段/12 秒提到 28 段/35 秒，日志请求量涨 3.5 倍，公共端点开始返回 429；每次 429 封禁 30 秒并把采集甩到 Alchemy 的 10 区块切片路径——正是 `ea84fb1` 修过的故障模式。两天后实测：游标滞后 58.8 万块、目录最新池 16.5 小时大、`analyzed=0` 且 `rpc_profile` 为空、24 小时可用分钟从 5387 掉到 1787、`alchemy_successes` 从 0 涨到 1542、`slow_successes` 从 5 涨到 3996。同一时刻直接探测公共端点 15/15 成功、中位 229ms，**说明限流来自 worker 自己的请求量**。

- 回退到 8 段/12 秒，**保留信号插队**（把同样的预算对准即将用到它的池，不花额外请求，一直是这个改动里有价值的那一半）。
...
2. 核实 `catalog_live_cursor` 是否已追平 `last_block`。本轮部署时滞后约 18 万块（约 5 小时），属积压非故障，每轮推进约 900–4500 块并在收敛；若长期不收敛再查 RPC。
3. 价格源现在有三个外部依赖（Coinbase ×2 + CoinGecko ×1）。CoinGecko 免费额度会 429；探测时若与生产 worker 同时打会触发。若 `failed_rounds` 明显上升，先看是不是这个。
4. 5% 成本闸、120 秒 ETH 新鲜度、稳定币脱锚带、策略规则现在都是冻结件，未获用户新的明确授权不得调整，也不得通过改仓位或成本模型间接绕过。
5. 在 Mac 上把 `65aed59`、`df34290` 推送到公开仓库 main。
