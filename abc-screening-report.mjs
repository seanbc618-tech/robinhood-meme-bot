...
function emptyFunnel() {
  return {unique_evals:0,observed:0,data_complete:0,age_ok:0,age_incomplete:0,observing:0,strategy_signal:0,safety_pass:0,safety_fail:0,paper_fill:0,no_t_aggregate:0,data_or_age_block:0,
    stages:{NOT_OBSERVED:0,OBSERVED:0,DATA_INCOMPLETE:0,AGE_INCOMPLETE:0,OBSERVING:0,NO_STRATEGY_SIGNAL:0,STRATEGY_SIGNAL:0,SAFETY_PASS:0,SAFETY_FAIL:0,PAPER_FILL:0},funnel:null};
}
...
export function screeningReport(store,{now=Date.now(),hours=24,readOnly=true}={}) {
  ...
  try {
    ...
    for(const r of rows) {
      ...
      if(r.funnel_stage==='OBSERVING') s.observing++;
      if(r.funnel_stage==='DATA_INCOMPLETE'||r.funnel_stage==='OBSERVING') s.data_or_age_block++;
    }
    for(const strat of ['A','B','C']) {
      const s=byStrat[strat];
      // data_complete excludes both DATA_INCOMPLETE and OBSERVING (observation not yet complete)
      s.data_complete=s.observed-s.stages.DATA_INCOMPLETE-s.stages.OBSERVING;
      s.age_ok=s.data_complete-s.stages.AGE_INCOMPLETE;
      s.funnel={
        observed:s.observed,
        ...
        denominators:{
          ...
          data_complete:'observed minus DATA_INCOMPLETE minus OBSERVING (history warmups + observation window)',
          age_satisfied:'data_complete minus AGE_INCOMPLETE (graduation-age gates only)',
          observing:'consecutive-minute / post-grad observation window incomplete (!= AGE, != generic DATA)',
